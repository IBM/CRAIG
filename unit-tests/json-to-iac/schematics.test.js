...
# Schematics Agent
##############################################################################
resource "ibm_schematics_agent" "agent" {
  agent_infrastructure {
    infra_type             = "ibm_openshift"
    cluster_id             = ibm_container_vpc_cluster.schematics_vpc_vpc_schematics_agent_cluster.id
    cluster_resource_group = ibm_resource_group.schematics_rg.id
    cos_instance_name      = ibm_resource_instance.cos.name
  ...
  description           = "Schematics agent for ISV Managed Services"
...
resource "ibm_schematics_agent_deploy" "agent_deploy" {
  agent_id = ibm_schematics_agent.agent.id
}
...
resource "ibm_schematics_policy" "policy" {
  description = "Policy to allow execution of actions on schematics-agent"
  name        = "schematics-agent-policy"
  ...
  parameter {
    agent_assignment_policy_parameter {
      selector_scope {
        kind            = "action"
        tags            = ["schematics-agent"]
        resource_groups = [ibm_resource_group.schematics_rg.id]
  ...
  target {
    ...
    selector_ids = [
      ibm_schematics_agent.agent.id
    ]
  }
  depends_on = [
    ibm_schematics_agent_deploy.agent_deploy
  ]
}
